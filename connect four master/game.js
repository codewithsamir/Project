...
function oncolumclick(column) {
    ...
    console.log(pieces.filter((_, index) => index % 7 === column))
    console.log(avialablerow)
    if (avialablerow === -1) {
        // no space in column 
        return;
    }
    console.log(pieces[(avialablerow * 7) + column])
    pieces[(avialablerow * 7) + column] = playerTurn;
    console.log(pieces[(avialablerow * 7) + column]  = playerTurn)
